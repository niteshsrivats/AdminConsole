...
class DepartmentsProvider extends Component {
  ...
  async componentDidMount() {
    const departments = this.getDepartmentFromObject(JSON.parse(localStorage.getItem("departments")));

    if (!!!departments) {
      await firebase.firestore().collection("departments").doc("all").get()
        .then((doc) => {
          localStorage.setItem("departments", JSON.stringify({
            departments: doc.data(),
            expiry: (new Date()).getTime() + this.ttl
          }));
          this.setState({ departments: doc.data() });
        })
